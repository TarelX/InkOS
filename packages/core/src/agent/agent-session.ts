...
import { buildRichAttachmentBlock, withNativeFilePayloads } from "../llm/file-input/index.js";
...
import {
  buildAttachmentDisplayMessage,
  compactAttachmentRefs,
  persistUserMessageWithAttachments,
} from "../interaction/attachment-transcript.js";
...
export interface AgentSessionConfig {
  ...
  /**
   * V2 (ported from V1 patch-thinking-level): explicit per-turn thinking level.
   * pi-ai maps SimpleStreamOptions.reasoning to reasoning_effort /
   * thinkingConfig per API. Mutable via a ref so later turns change level
   * without rebuilding the cached Agent.
   */
  reasoning?: "minimal" | "low" | "medium" | "high" | "xhigh";
  /**
   * V2 (ported from V1 patch-cursor-mode): private `cursor_mode` field for the
   * local Cursor AgentService reverse proxy. The HOST decides whether the
   * selected endpoint is the Cursor proxy — only then is this set; every other
   * endpoint must keep a clean payload.
   */
...
export interface AgentSessionAttachment {
  ...
  /** 宿主解析失败原因（rich attachments：如实告知模型，不得编造内容）。 */
  readonly extractError?: string;
  /** 宿主解析出的分节内容（Office/PDF/表格，见 materials/document-extract）。 */
  readonly extracted?: {
    readonly kind: string;
    readonly sections: ReadonlyArray<{ readonly id: string; readonly title: string; readonly text: string; readonly truncated?: boolean }>;
...
interface CachedAgent {
  ...
  suppressProductionTools: boolean;
  /** V2 思考强度：streamFn 闭包读取该 ref，后续轮次改级别无需重建 Agent。 */
  reasoningRef: { level: AgentSessionConfig["reasoning"] };
  /** V2 Cursor 反代模式：仅 host 判定为本地反代时有值。 */
  cursorModeRef: { mode: AgentSessionConfig["cursorMode"] };
...
async function runAgentSessionUnlocked(
  ...
  if (!cached) {
    ...
    // V2（源码移植自 V1 patch-thinking-level / patch-cursor-mode）：
    // 可变 ref 让 streamFn 闭包在后续轮次读取到新的思考强度 / 反代模式。
    const reasoningRef: CachedAgent["reasoningRef"] = { level: config.reasoning };
    const cursorModeRef: CachedAgent["cursorModeRef"] = { mode: config.cursorMode };
    const agent = new Agent({
      ...
      // V1 patch-architect-retry：architect_incomplete 必须以工具错误呈现，
      // 模型才不会把"建书失败"当成功继续走。
      afterToolCall: async ({ result }) => {
        const kind = (result?.details as { kind?: unknown } | undefined)?.kind;
        return kind === "architect_incomplete" ? { isError: true } : undefined;
      },
      ...
      streamFn: (streamModel, context, options) => {
        ...
        // 显式思考强度：在模型克隆上开 reasoning 并钉住 system 角色（部分
        // OpenAI 兼容端点不认 developer 角色——Moonshot 教训）。pi-ai 把
        // options.reasoning 映射为各 API 的 reasoning_effort / thinkingConfig。
        const reasoningLevel = reasoningRef.level;
        const effectiveModel = reasoningLevel
          ? ({
              ...streamModel,
              reasoning: true,
              compat: {
                ...((streamModel as { compat?: Record<string, unknown> }).compat ?? {}),
                supportsDeveloperRole: false,
                ...(reasoningLevel === "xhigh"
                  ? {
                      reasoningEffortMap: {
                        ...(((streamModel as { compat?: { reasoningEffortMap?: Record<string, string> } }).compat?.reasoningEffortMap) ?? {}),
                        xhigh: "xhigh",
                      },
                    }
                  : {}),
              },
            } as typeof streamModel)
          : streamModel;
        // Cursor 反代私有 cursor_mode：仅当 host 判定端点为本地反代时注入请求体。
        const cursorMode = cursorModeRef.mode;
        ...
        const effectiveOptions = {
          ...
          ...(cursorMode
            ? {
                onPayload: async (payload: unknown, payloadModel: typeof streamModel) => {
                  const prior = priorOnPayload ? await priorOnPayload(payload, payloadModel) : undefined;
                  const base = prior ?? payload;
                  if (base && typeof base === "object" && !Array.isArray(base)) {
                    return { ...(base as Record<string, unknown>), cursor_mode: cursorMode };
                  }
                  return prior;
                },
              }
        ...
        // 原生附件 marker 在 payload 边界物化为各协议的文件块（PDF 直传等）。
        return guardedStreamSimple(effectiveModel, context, withNativeFilePayloads(effectiveOptions, { projectRoot }));
      },
  ...
  // 每轮更新思考强度 / 反代模式（ref 可变，无需重建缓存的 Agent）。
  cached.reasoningRef.level = config.reasoning;
  cached.cursorModeRef.mode = config.cursorMode;
  cached.turnSkillIds.clear();
  for (const skillId of skillResolution.forcedSkillIds) {
    cached.turnSkillIds.add(skillId);
  }
  const { agent } = cached;
  // Rich attachment block：PDF 按端点能力走原生直传 marker，解析文档按预算
  // 相关性注入，纯文本直通（V1 patch-rich-attachments 行为）。
  const attachmentBlock = await buildRichAttachmentBlock(config.attachments, language, {
    model,
    projectRoot,
    userMessage,
    apiKey: config.apiKey,
  }).catch(() => buildAttachmentUserBlock(config.attachments, language));
  const promptMessage = attachmentBlock ? `${userMessage}${attachmentBlock}` : userMessage;
  // 展开的附件正文只给模型；会话记录/UI 持久化用户原文 + 精简文件名摘要
  //（V1 patch-attachment-transcript-separation）。
  const attachmentRefs = compactAttachmentRefs(config.attachments);
  const displayMessage = buildAttachmentDisplayMessage(userMessage, attachmentRefs, language);
  ...
  const persistAgentEvent = async (event: AgentEvent): Promise<void> => {
    ...
    const sanitizedMessage = sanitizeSkillTurnMessage(event.message, skillTurnActive);
    // 用户消息落盘时替换为精简版（原文 + 文件引用），附件正文不进会话记录
    const persistedMessage = role === "user" && attachmentRefs.length > 0
      ? persistUserMessageWithAttachments(sanitizedMessage, displayMessage, attachmentRefs)
      : sanitizedMessage;
