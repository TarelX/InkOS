...
async function extractBufferMaterial(
  ...
  // Office 格式（xlsx/xls/csv/tsv/docx/pptx/odt/odp/ods/rtf）走统一文档抽取器，
  // 让素材导入与聊天上传的可读格式保持一致（V1 patch-rich-attachments）。
  const extraction = await extractDocument({ buffer, filename: meta.filename, mimeType });
  if (extraction.ok) {
    return {
      kind: "text",
      source: meta.source,
      title: stripExtension(meta.filename),
      mimeType,
      text: extraction.sections.map((section) => `## ${section.title}\n\n${section.text}`).join("\n\n"),
    };
  }
  throw new Error(extraction.error ?? `Unsupported material type: ${mimeType || meta.filename}`);
}
