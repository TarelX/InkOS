  ...
  private async rebuildCurrentStateFactHistory(bookDir: string, uptoChapter: number): Promise<void> {
    const memoryDb = await this.withMemoryIndexRetry(async () => {
      ...
      try {
        ...
        // 保留从全量章节摘要历史回填的具名角色时序事实（V1
        // patch-memory-backfill-persistence）：上面的快照重放只能重建六个
        // 粗粒度 protagonist 字段，否则每次完稿都会抹掉有用的角色级记忆。
        const backfillRaw = await readFile(join(bookDir, "story", "state", "facts-backfill.json"), "utf-8").catch(() => "");
        if (backfillRaw.trim()) {
          const parsed = JSON.parse(backfillRaw) as { facts?: unknown[] } | null;
          const backfillFacts = Array.isArray(parsed?.facts) ? parsed.facts : [];
          for (const raw of backfillFacts) {
            const fact = raw as {
              subject?: unknown; predicate?: unknown; object?: unknown;
              validFromChapter?: unknown; validUntilChapter?: unknown; sourceChapter?: unknown;
            } | null;
            if (!fact || typeof fact.subject !== "string"
              || typeof fact.predicate !== "string"
              || typeof fact.object !== "string"
              || !Number.isInteger(fact.validFromChapter)
              || !Number.isInteger(fact.sourceChapter)) {
              continue;
            }
            db.addFact({
              subject: fact.subject,
              predicate: fact.predicate,
              object: fact.object,
              validFromChapter: fact.validFromChapter as number,
              validUntilChapter: Number.isInteger(fact.validUntilChapter)
                ? (fact.validUntilChapter as number)
                : null,
              sourceChapter: fact.sourceChapter as number,
            });
          }
        }
