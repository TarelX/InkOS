...
export function pickModelSelection(
  ...
  // V1 patch-refresh-state（v2 语义）：用户已有选择永远优先，立即短路 ——
  // 自定义服务（如本地反代）的模型枚举不出来也不允许被"回退"覆盖。
  ...
  // 持久化配置只用来播种初始空选择；service+model 都有时，即使枚举不到
  // 也按配置原样恢复（自定义服务模型列表可能拉不到）。
  if (preferredService && preferredModel) {
    const preferredGroup = groupedModels.find((group) => group.service === preferredService);
    const exactModel = preferredGroup?.models.find((model) => model.id === preferredModel);
    return preferredGroup && exactModel
      ? { model: exactModel.id, service: preferredGroup.service }
      : { model: preferredModel, service: preferredService };
  }
  if (preferredService) {
    const preferredGroup = groupedModels.find((group) => group.service === preferredService);
