...
export function analyzeStyle(
  ...
  // 中文网文一行一段（常带全角缩进），英文散文用空行分段（V1 patch-style-analyzer）。
  // 统计前丢掉 Markdown 标题/表格分隔线，避免污染风格数据。
  const normalizedText = text.replace(/\r\n?/g, "\n");
  const isContentUnit = (unit: string): boolean => unit.length > 0
    && !/^#{1,6}\s/.test(unit)
    && !/^[-|: ]{3,}$/.test(unit);
  const paragraphs = normalizedText
    .split(isEn ? /\n\s*\n/ : /\n+/)
    .map((p) => p.trim())
    .filter(isContentUnit);
