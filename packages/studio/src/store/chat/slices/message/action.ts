...
export const createMessageSlice: StateCreator<ChatStore, [], [], MessageActions> = (set, get) => ({
  activateSession: (sessionId) => {
    // V1 patch-refresh-state：记住每本书最后激活的会话，刷新后恢复到同一会话
    if (sessionId) {
      try {
        const session = get().sessions[sessionId];
        window.localStorage.setItem(`inkos:last-session:${bookKey(session?.bookId ?? null)}`, sessionId);
  ...
  setSelectedModel: (model, service) => {
    ...
    // V1 patch-refresh-state：选择即持久化到项目配置，刷新后不丢
    if (model && service) {
      void fetchJson("/services/config", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ service, defaultModel: model, configSource: "studio" }),
      }).catch(() => { /* 持久化失败不阻断选择 */ });
    }
  },
  ...
  setThinkingLevel: (level) => {
    try {
      if (level) window.localStorage.setItem("inkos:chat:thinking-level", level);
      else window.localStorage.removeItem("inkos:chat:thinking-level");
    } catch {
  ...
  setInteractionMode: (mode) => {
    try {
      window.localStorage.setItem("inkos:chat:interaction-mode", mode);
    } catch {
      // storage unavailable — keep in-memory only
    }
  ...
  loadSessionDetail: async (sessionId) => {
    ...
    try {
      ...
      // chatRunning（V1 patch-refresh-stop-button）：普通聊天轮没有任务快照，
      // 刷新后靠服务端在飞计数恢复停止按钮与 SSE 流。
      const taskRunning = data.chatRunning === true
      ...
      if (taskRunning) {
        ...
        attachSessionStreamListeners({
          sessionId: detailSessionId,
          // 无任务快照的聊天轮用当前时间作流锚点
          streamTs: task?.execution.startedAt ?? Date.now(),
          streamEs,
          set,
          get,
        });
