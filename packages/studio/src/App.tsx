...
import { IconRail, DEFAULT_RAIL_ICONS } from "./components/IconRail";
...
const BookWorkbench = lazy(() => import("./pages/BookWorkbench").then((m) => ({ default: m.BookWorkbench })));
...
export function App() {
  ...
  // 四栏工作台的配色按设计稿固定为浅色（深海军蓝顶栏 + 白卡片）。内嵌的
  // ChatPage 走全局 dark 变体，若此时文档仍是暗色，第二列会与其余三列撞色，
  // 所以工作台路由期间统一压成浅色；离开路由后恢复用户的主题选择。
  const forceLightForWorkbench = route.page === "book" || route.page === "adaptation";
  useEffect(() => {
    document.documentElement.classList.toggle("dark", isDark && !forceLightForWorkbench);
  }, [isDark, forceLightForWorkbench]);
  ...
  const nav = {
    ...
    toAdaptation: (bookId: string) => setRoute({ page: "adaptation", bookId }),
  };
  const homeNav = {
    toDashboard: nav.toDashboard,
    toBook: nav.toBook,
    toBookCreate: nav.toBookCreate,
    toChat: nav.toChat,
    toImport: nav.toImport,
    toTranslation: nav.toTranslation,
    toFilmStudio: nav.toFilmStudio,
  };
  ...
  // V2 外壳：dashboard 走新首页（HomePage + 图标栏，无全局 header）；
  // 其余页面保留 IconRail + 顶栏；immersive 页面（chat/book/v2 工作台/影游/翻译…）
  // 渲染自己的页头，只保留图标栏。
  const homePage = route.page === "dashboard";
  // 四栏工作台（书籍页与 #/v2 别名）自带深蓝顶栏与文件管理列：
  // 隐藏全局 header 和旧会话侧栏，避免双层导航。
  const workbenchPage = route.page === "book" || route.page === "adaptation";
  const fullImmersive = workbenchPage || route.page === "chat" || route.page === "book-create";
  const hideChrome = homePage || workbenchPage;

  const railItems = [
    { id: "dashboard", title: "首页", icon: DEFAULT_RAIL_ICONS.home, onClick: nav.toDashboard },
    { id: "chat", title: "项目对话", icon: DEFAULT_RAIL_ICONS.play, onClick: nav.toChat },
    { id: "book", title: "我的创作", icon: DEFAULT_RAIL_ICONS.novel, onClick: () => (activeBookId ? nav.toBook(activeBookId) : nav.toDashboard()) },
    { id: "import", title: "导入", icon: DEFAULT_RAIL_ICONS.adaptation, onClick: () => nav.toImport() },
    { id: "style", title: "文风", icon: DEFAULT_RAIL_ICONS.storyboard, onClick: nav.toStyle },
    { id: "translation", title: "翻译译介", icon: DEFAULT_RAIL_ICONS.translation, onClick: nav.toTranslation },
    { id: "radar", title: "市场雷达", icon: DEFAULT_RAIL_ICONS.script, onClick: nav.toRadar },
    { id: "doctor", title: "环境诊断", icon: DEFAULT_RAIL_ICONS.short, onClick: nav.toDoctor },
  ];

  return (
    <div className="h-screen bg-background text-foreground flex overflow-hidden font-sans">
      {/* V2 narrow icon rail (all pages) */}
      <IconRail
        activePage={activePage}
        items={railItems}
        onSettings={nav.toProjectSettings}
        theme={isDark ? "dark" : "light"}
        onToggleTheme={() => setTheme(isDark ? "light" : "dark")}
      />
      {/* Full session sidebar: hidden on the new home page */}
      {!hideChrome && <Sidebar nav={nav} activePage={activePage} sse={sse} t={t} />}

      {/* Center Content */}
      <div className="flex-1 flex flex-col min-w-0 bg-background/30 backdrop-blur-sm">
        {/* Header Strip (hidden on home + immersive pages) */}
        {!hideChrome && !fullImmersive && (
        <header className="h-14 shrink-0 flex items-center justify-between px-8 border-b border-border/40">
          ...
          <div className="flex items-center gap-3">
            {activeBookId && (
              <button
                onClick={() => nav.toAdaptation(activeBookId)}
                className="rounded-lg border border-primary/40 bg-primary/10 px-3 py-1.5 text-[14px] font-medium text-primary hover:bg-primary/20 transition-colors"
              >
                V2 改编工作台
              </button>
            )}
        ...
        <main className="flex-1 relative overflow-y-auto scroll-smooth">
          {route.page === "dashboard" && (
            <div className="h-full overflow-y-auto">
              <HomePage nav={homeNav} theme={theme} t={t} />
            </div>
          )}
          ...
          {/* 书籍页 = V2 四栏工作台（原创 / 改编共用同一界面）。 */}
          {route.page === "book" && (
            <div className="absolute inset-0 flex min-w-0">
              <Suspense fallback={<div className="m-auto w-10 h-10 border-4 border-primary/20 border-t-primary rounded-full animate-spin" />}>
                <BookWorkbench bookId={route.bookId} nav={nav} theme={theme} t={t} sse={sse} />
              </Suspense>
            </div>
          )}
          {/* #/v2/:id 保留为工作台的直达别名。 */}
          {route.page === "adaptation" && (
            <div className="absolute inset-0 flex min-w-0">
              <Suspense fallback={<div className="m-auto w-10 h-10 border-4 border-primary/20 border-t-primary rounded-full animate-spin" />}>
                <BookWorkbench bookId={route.bookId} nav={nav} theme={theme} t={t} sse={sse} />
              </Suspense>
