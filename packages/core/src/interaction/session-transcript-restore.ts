...
import { restoredAttachmentReferenceBlock } from "./attachment-transcript.js";
...
/**
 * UI 可见消息（V1 patch-refresh-state）：与 committedMessageEvents 不同，
 * 未提交轮次的消息也保留（刷新不丢正在进行的内容），且 request_failed
 * 合成一条 "✗ 错误" 的 assistant 消息，用户能看到那一轮到底怎么死的。
 */
export function visibleMessageEvents(events: TranscriptEvent[], sessionKind?: SessionKind): MessageEvent[] {
  const requestKinds = new Map(
    events
      .filter((event) => event.type === "request_started")
      .map((event) => [event.requestId, event.sessionKind]),
  ...
  const isVisibleRequest = (requestId: string): boolean => {
    if (!sessionKind) return true;
    const kind = requestKinds.get(requestId);
    return kind === undefined || kind === sessionKind;
  };
  const visible = events.filter(
    (event): event is MessageEvent => event.type === "message" && isVisibleRequest(event.requestId),
  );
  for (const event of events) {
    if (event.type !== "request_failed" || !isVisibleRequest(event.requestId)) continue;
    const error = typeof event.error === "string" && event.error.trim() ? event.error.trim() : "Request failed";
    const alreadyVisible = visible.some((messageEvent) =>
      messageEvent.requestId === event.requestId
      && messageEvent.role === "assistant"
      && textFromContent((messageEvent.message as { content?: unknown } | null)?.content).includes(error));
    ...
    visible.push({
      ...
      uuid: `restore-error-${event.requestId}`,
      parentUuid: null,
      role: "assistant",
      piTurnIndex: Number.MAX_SAFE_INTEGER,
      message: {
        role: "assistant",
...
function textOnlyAgentMessage(event: MessageEvent): AgentMessage | null {
  ...
  if (event.role === "user" || event.role === "system") {
    ...
    // 只有恢复的模型上下文能拿到持久文件路径；Studio UI 直接从 raw.content
    // 派生可见消息，永远看不到这段（V1 patch-attachment-transcript-separation）。
    const attachmentContext = event.role === "user" ? restoredAttachmentReferenceBlock(raw) : "";
    const restoredContent = content ? `${content}${attachmentContext}` : attachmentContext.trim();
    return restoredContent ? { role: event.role, content: restoredContent, timestamp: event.timestamp } as AgentMessage : null;
  }
...
export async function deriveBookSessionFromTranscript(
  ...
  // UI 记录用可见消息（含未提交轮 + 失败标记），模型上下文恢复仍走 committed
  const messages = messageEventsToInteractionMessages(visibleMessageEvents(events, sessionKind));
