...
// Lexical tells adapted from Novel Writer's MIT-licensed writing rules.
// One occurrence can be natural; repeated stock phrasing is what we flag.
const AI_LEXICAL_TELLS_ZH: ReadonlyArray<string> = [
  "弥漫着", "摇摇欲坠", "话音未落", "心中暗想", "不由得",
  "下意识地", "空气仿佛凝固", "嘴角勾起", "眼中闪过一丝",
  "一股莫名", "不禁让人", "仿佛在诉说", "陷入沉思",
];
...
export function analyzeAITells(content: string, language: AITellLanguage = "zh"): AITellResult {
  ...
  // dim 24: repeated stock lexical tells（中文特供，V1 patch-hooks-pipeline）
  if (!isEnglish && totalChars > 0) {
    const lexicalCounts: Record<string, number> = {};
    let lexicalTotal = 0;
    for (const word of AI_LEXICAL_TELLS_ZH) {
      const count = content.split(word).length - 1;
      if (count > 0) {
        lexicalCounts[word] = count;
        lexicalTotal += count;
      }
    }
    const repeated = Object.entries(lexicalCounts).filter(([, count]) => count >= 2);
    const density = lexicalTotal / (totalChars / 1000);
    // 密度阈值只对成章文本有意义：短文本里单次出现是自然用词，不当套语抓
    if (repeated.length > 0 || (totalChars >= 1000 && density > 2.5)) {
      const detail = Object.entries(lexicalCounts)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 8)
        .map(([word, count]) => `"${word}"×${count}`)
        .join("、");
      issues.push({
        severity: "warning",
        category: "AI高频词",
        description: `检测到模板化AI词组：${detail}；密度${density.toFixed(1)}次/千字`,
        suggestion: "删去套语并改成可观察的动作、物件变化、具体身体反应或环境因果",
      });
    }
  }
