...
// 20MB：覆盖常见 Office/PDF 文档（原生 PDF 直传上限 15MB + 余量）；
// 注入文本仍受 MAX_AGENT_ATTACHMENT_TEXT_CHARS 与上下文预算约束。
...
/**
 * Rich document extraction（V1 patch-rich-attachments 移植）：表格/Word/幻灯片/PDF
 * 在宿主侧解析成分节文本，任何模型（包括纯 OpenAI 兼容反代）都能读；原始文件
 * 留在磁盘供原生文件协议直传。解析失败变成显式 extractError，而不是静默存一个
 * 模型读不了的文件。
 */
async function extractRichAttachment(root: string, args: {
  buffer: Buffer;
  filename: string;
  mimeType: string;
  ...
  const { buffer, filename, mimeType, storedRelPath, storedAbsPath, id, size } = args;
  const base = { id, filename, mimeType, size, storedPath: storedRelPath };
  let extraction: Awaited<ReturnType<typeof extractDocument>>;
  try {
    extraction = await extractDocument({ buffer, filename, mimeType });
  } catch (error) {
    return { ...base, extractError: `解析异常：${error instanceof Error ? error.message : String(error)}` };
  }
  if (!extraction.ok) {
    return { ...base, extractError: extraction.error };
  }
  try {
    // 解析产物落盘（尽力而为）：便于用户/后续会话直接查看抽取结果
    await writeFile(`${storedAbsPath}.extracted.md`, renderExtractedMarkdown(filename, extraction), "utf-8");
  } catch { /* persistence is best effort; sections still travel in memory */ }
  return {
    ...base,
    extracted: {
...
async function normalizeAgentAttachments(
  ...
  for (const [index, raw] of value.entries()) {
    ...
    const richKind = classifyAttachmentKind(filename, mimeType);
    if (richKind === "spreadsheet" || richKind === "word" || richKind === "slides" || richKind === "pdf") {
      out.push(await extractRichAttachment(root, {
        buffer: parsed.buffer,
        filename,
        mimeType,
        storedRelPath: relPath,
        storedAbsPath: storedPath,
        id: payload.id || `${Date.now()}-${index}`,
        size: parsed.buffer.byteLength,
      }));
      continue;
    ...
    if (isTextAttachment(filename, mimeType) || richKind === "text") {
      // 智能解码：BOM/二进制检测/GBK 回退，中文文本不再乱码
      const decoded = decodeTextSmart(parsed.buffer);
      if (decoded.binary) {
        out.push({
          id: payload.id || `${Date.now()}-${index}`,
          filename,
          ...
          size: parsed.buffer.byteLength,
          storedPath: relPath,
          extractError: decoded.warnings[0] ?? "文件不是可读文本",
        });
        continue;
      }
      if (decoded.text.length > MAX_AGENT_ATTACHMENT_TEXT_CHARS) {
        // 整本小说这类超大文本：不注入正文也不报错 —— 文件已落盘，
        // 给模型开头节选 + 明确指引，用 stored_path 走 read/import_chapters
        // 分段读取或导入（导入原著的主路径）。
        const head = decoded.text.slice(0, 2_000);
        out.push({
          id: payload.id || `${Date.now()}-${index}`,
          filename,
          mimeType,
          ...
          extracted: {
            ...
            sections: [{
              ...
              title: filename,
              text: [
                `【大文件说明】全文共 ${decoded.text.length.toLocaleString()} 字符，超过内联注入上限，正文未注入本条消息。`,
                `文件已保存到 stored_path（见上方）。需要内容时用 read 工具按路径分段读取；要导入为书籍章节时用 import_chapters 工具并传 sourcePath=stored_path。`,
                "不得凭空杜撰未读取的内容。",
                "",
                "【开头节选（前 2000 字符）】",
                head,
            ...
            meta: { oversized: true, encoding: decoded.encoding },
          },
        });
        continue;
      }
    ...
    out.push({
      ...
      size: parsed.buffer.byteLength,
      storedPath: relPath,
      extractError: `暂不支持解析该格式（${mimeType || filename}），文件已保存但模型无法读取内容`,
    });
  }
  return out;
}
...
// --- V2：思考强度与 Cursor 反代模式（源码移植自 V1 patch-thinking-level / patch-cursor-mode） ---

export type ThinkingLevel = "minimal" | "low" | "medium" | "high" | "xhigh";

/**
 * 解析思考强度指令。优先级：请求体显式 thinkingLevel > "/think <level>" 指令
 * 前缀（单轮） > "model@<level>" 模型后缀（持久）。返回清洗后的 instruction/model。
 */
export function parseThinkingDirective(
  instruction: string | undefined,
  model: string | undefined,
): { instruction: string; model: string | undefined; level: ThinkingLevel | undefined; fromModelSuffix: boolean } {
  const ZH_LEVELS: Record<string, ThinkingLevel> = {
    最低: "minimal", 低: "low", 中: "medium", 高: "high", 极高: "xhigh", 最高: "xhigh",
  };
  let level: ThinkingLevel | undefined;
  let fromModelSuffix = false;
  let cleanModel = model;
  if (typeof model === "string") {
    const suffix = model.match(/^(.+)@(minimal|low|medium|high|xhigh)$/i);
    if (suffix) {
      cleanModel = suffix[1];
      level = suffix[2].toLowerCase() as ThinkingLevel;
      fromModelSuffix = true;
    }
  }
  let cleanInstruction = typeof instruction === "string" ? instruction : "";
  // (?:\s+|$) 而不是 \b：JS 词边界只认 ASCII，中文级别词（高/极高）用 \b 永远匹配不上。
  const prefix = cleanInstruction.match(
    /^\/(?:think|thinking|思考)\s*[:：]?\s*(minimal|low|medium|high|xhigh|最低|极高|最高|低|中|高)(?:\s+|$)/i,
  );
  if (prefix) {
    level = ZH_LEVELS[prefix[1]] ?? (prefix[1].toLowerCase() as ThinkingLevel);
    cleanInstruction = cleanInstruction.slice(prefix[0].length).trim();
  }
  return { instruction: cleanInstruction, model: cleanModel, level, fromModelSuffix };
}

/** cursor_mode 是本地 Cursor 反代的私有字段：只有该端点可以收到，判定看服务 id 与 baseUrl。 */
export function isCursorProxyService(serviceId: string | undefined, baseUrl: string | undefined): boolean {
  const id = String(serviceId ?? "").toLowerCase();
  if (id.includes("cursor")) return true;
  return String(baseUrl ?? "").toLowerCase().includes("cursor");
}

// ---- V1 patch-direct-chapter-actions：口语化审计/修订命令的确定性解析 ----

export interface StudioDirectChapterAction {
  readonly intent: "audit_chapter" | "revise_chapter";
  readonly chapterNumber?: number;
  readonly mode?: ReviseMode;
}

function parseChineseChapterInteger(raw: string): number | undefined {
  const text = String(raw ?? "").trim();
  if (/^\d+$/.test(text)) return Number(text);
  const digits: Record<string, number> = { "零": 0, "〇": 0, "一": 1, "二": 2, "两": 2, "三": 3, "四": 4, "五": 5, "六": 6, "七": 7, "八": 8, "九": 9 };
  if (/^[零〇一二两三四五六七八九]$/.test(text)) return digits[text];
  const hundredParts = text.split("百");
  let value = 0;
  let rest = text;
  if (hundredParts.length === 2) {
    value += (digits[hundredParts[0]!] ?? 1) * 100;
    rest = hundredParts[1]!;
  }
  if (rest.includes("十")) {
    const [left, right] = rest.split("十");
    value += (left ? (digits[left] ?? 0) : 1) * 10;
    value += right ? (digits[right] ?? 0) : 0;
  } else if (rest) {
    value += digits[rest] ?? 0;
  }
...
function explicitChapterNumber(instruction: string): number | undefined {
  const zh = instruction.match(/第\s*([0-9零〇一二两三四五六七八九十百]+)\s*章/i);
  if (zh) return parseChineseChapterInteger(zh[1]!);
  const en = instruction.match(/\bchapter\s*(\d+)\b/i);
  return en ? Number(en[1]) : undefined;
}

/**
 * Commands that have a deterministic InkOS operation must not depend on a
 * third-party model deciding whether to emit a tool call. Questions about
 * past execution remain ordinary chat turns.
 */
export function parseStudioDirectChapterAction(instruction: string): StudioDirectChapterAction | null {
  const text = String(instruction ?? "").trim();
  if (!text) return null;
  // "你重写了第二章吗" is a status question; "能重写第二章吗" is a soft command.
  const statusQuestion = /(?:改|修订|重写|改写|润色).{0,12}了.*(?:吗|么|嘛)|(?:了吗|过吗|完成了吗|是否完成)|有没有.*(?:改|修订|重写)|^(?:我)?(?:没有|没|并未)(?:改|修订|重写)/i.test(text);
  if (statusQuestion) return null;
  const chapterNumber = explicitChapterNumber(text);
  if (/(?:审计|审核|审查)(?:一下|最新|第|这|刚才|上面|章节|$)/i.test(text)) {
    return { intent: "audit_chapter", chapterNumber };
  }
  // Broad spoken Chinese: "修改第二章/改一下第2章/把第二章改了" must not
  // depend on the chat model emitting sub_agent (third-party APIs often won't).
  if (/(?:修改|修订|重写|改写|润色|返工|修复|改改|改一下|改下)(?:一下|下)?(?:第|这|刚才|上面|最新|章节|啊|吧|$)|(?:把|将|帮我|请|能不能|可以|麻烦).*?(?:修改|修订|重写|改写|润色|修复|改一下|改下|改改|改了)|(?:第\s*[0-9零〇一二两三四五六七八九十百]+\s*章).{0,24}(?:修改|修订|重写|改写|润色|返工|修复|改一下|改下|改改|改了)|(?:修改|修订|重写|改写|润色|返工|修复|改一下|改下|改改).{0,24}(?:第\s*[0-9零〇一二两三四五六七八九十百]+\s*章)/i.test(text)) {
    const mode: ReviseMode = /反\s*AI|去\s*AI|anti.?detect/i.test(text)
      ? "anti-detect"
      : /润色|polish/i.test(text)
        ? "polish"
        : /返工|rework/i.test(text)
          ? "rework"
          : /重写|改写|rewrite/i.test(text)
            ? "rewrite"
            : "spot-fix";
    return { intent: "revise_chapter", chapterNumber, mode };
  }
  return null;
}
...
/** 直连审计工具（V1 patch-direct-chapter-actions）：确定性调 pipeline.auditDraft。 */
function createDirectAuditChapterTool(pipeline: PipelineRunner, bookId: string, chapterNumber: number | undefined, lang: StudioLanguage) {
  return {
    name: "sub_agent",
    async execute(
      _toolCallId: string,
      ...
      onUpdate?.({
        content: [{ type: "text", text: pick(lang, `正在审计第${chapterNumber ?? "最新"}章…`, `Auditing chapter ${chapterNumber ?? "latest"}...`) }],
      });
      const result = await pipeline.runWithAbortSignal(signal, () => pipeline.auditDraft(bookId, chapterNumber));
      const issues = Array.isArray(result.issues) ? result.issues : [];
      const summary = pick(
        lang,
        `审计第${result.chapterNumber}章：${result.passed ? "通过" : "未通过"}，发现 ${issues.length} 项问题。`,
        `Audit chapter ${result.chapterNumber}: ${result.passed ? "passed" : "failed"}, ${issues.length} issue(s).`,
      );
      const details = issues.slice(0, 12).map((issue) => `[${issue.severity ?? "info"}] ${issue.category ?? ""}: ${issue.description ?? ""}`).join("\n");
      return {
        content: [{ type: "text" as const, text: details ? `${summary}\n${details}` : summary }],
        details: { kind: "chapter_audit", bookId, chapterNumber: result.chapterNumber, passed: result.passed, issueCount: issues.length },
      };
    },
  };
}

/** 直连修订工具（V1 patch-direct-chapter-actions）：确定性调 pipeline.reviseDraft。 */
function createDirectReviseChapterTool(
  pipeline: PipelineRunner,
  bookId: string,
  ...
  return {
    ...
    async execute(
      ...
      onUpdate?.({
        content: [{ type: "text", text: pick(lang, `正在${mode === "rewrite" ? "重写" : "修订"}第${chapterNumber ?? "最新"}章…`, `${mode === "rewrite" ? "Rewriting" : "Revising"} chapter ${chapterNumber ?? "latest"}...`) }],
      });
      const result = await pipeline.runWithAbortSignal(signal, () => pipeline.reviseDraft(bookId, chapterNumber, mode, instruction));
      const fixed = Array.isArray(result.fixedIssues) ? result.fixedIssues : [];
      const text = pick(
        lang,
        `${result.applied ? "已应用" : "未应用"}第${result.chapterNumber}章${mode === "rewrite" ? "重写" : "修订"}，字数 ${result.wordCount}，状态 ${result.status}。${fixed.length ? `\n已处理：\n${fixed.slice(0, 12).join("\n")}` : ""}`,
        `${result.applied ? "Applied" : "Did not apply"} ${mode} for chapter ${result.chapterNumber}; ${result.wordCount} words, status ${result.status}.${fixed.length ? `\nFixed:\n${fixed.slice(0, 12).join("\n")}` : ""}`,
      );
      return {
        ...(!result.applied ? { isError: true } : {}),
        content: [{ type: "text" as const, text }],
        details: { kind: "chapter_revision", bookId, chapterNumber: result.chapterNumber, mode, applied: result.applied, status: result.status, wordCount: result.wordCount, fixedIssues: fixed },
      };
    },
  };
}
...
async function executeConfirmedProductionAction(args: {
  ...
  /** V1 patch-direct-chapter-actions：解析出的直连章节动作（章号/修订模式）。 */
  readonly directChapterAction?: StudioDirectChapterAction | null;
  ...
  let tool: ReturnType<typeof createSubAgentTool>
    ...
    | ReturnType<typeof createDirectAuditChapterTool>
    | ReturnType<typeof createDirectReviseChapterTool>;
  ...
  } else if (args.requestedIntent === "audit_chapter") {
    if (!args.bookId) {
      throw new ApiError(400, "BOOK_ID_REQUIRED", pick(lang, "审计需要先打开一本书。", "Auditing requires an active book."));
    }
    const direct: Partial<StudioDirectChapterAction> = args.directChapterAction ?? {};
    tool = createDirectAuditChapterTool(args.pipeline, args.bookId, direct.chapterNumber, lang);
    agent = "auditor";
    params = { agent, ...(direct.chapterNumber ? { chapterNumber: direct.chapterNumber } : {}) };
  } else if (args.requestedIntent === "revise_chapter") {
    if (!args.bookId) {
      throw new ApiError(400, "BOOK_ID_REQUIRED", pick(lang, "修订需要先打开一本书。", "Revision requires an active book."));
    }
    const direct: Partial<StudioDirectChapterAction> = args.directChapterAction ?? {};
    const mode = direct.mode ?? "spot-fix";
    tool = createDirectReviseChapterTool(args.pipeline, args.bookId, direct.chapterNumber, mode, args.instruction, lang);
    agent = "reviser";
    params = { agent, mode, ...(direct.chapterNumber ? { chapterNumber: direct.chapterNumber } : {}) };
...
function formatServiceProbeError(args: {
  ...
  // V1 patch-model-chat-test：中转账号池耗尽（如 cursor 反代 503）给出专属诊断，
  // 不再让用户误以为是 API Key 填错。
  if (/no available accounts?/i.test(rawDetail)) {
    return [
      pick(lang, `${args.label ?? args.service} 的上游账户池当前没有可用账号。`, `${args.label ?? args.service}'s upstream account pool currently has no available accounts.`),
      context,
      "",
      pick(lang, "模型列表探测已通过，但该模型的最小真实推理请求被上游以 503 拒绝。", "The model-list probe passed, but upstream rejected the minimal real inference request with HTTP 503."),
      pick(lang, "这不是 API Key 错误；请切换模型或服务商，或等待中转服务补充可用账号后重试。", "This is not an API Key error. Switch the model/provider, or retry after the relay replenishes its account pool."),
...
/**
 * 对指定模型发一次最小真实推理请求（V1 patch-model-chat-test）：
 * 模型列表探测通过 ≠ 模型能用（典型：中转账号池限流 503）。
 */
async function runServiceChatProbe(args: {
  service: string;
  apiKey: string;
  baseUrl: string;
  model: string;
...
}): Promise<{ ok: boolean; model: string; apiFormat?: "chat" | "responses"; stream?: boolean; error?: string }> {
  const lang = args.language ?? "zh";
  ...
  try {
    await withTimeout(
      chatCompletion(client, args.model, [{ role: "user", content: "Reply with OK only." }], { maxTokens: 16, retry: false }),
      SERVICE_CHAT_PROBE_TIMEOUT_MS,
      "service chat test",
      lang,
    );
    return { ok: true, model: args.model, apiFormat: args.apiFormat, stream: args.stream };
  } catch (error) {
    return {
      ok: false,
      model: args.model,
...
async function probeServiceCapabilities(args: {
  ...
  const endpoint = getAllEndpoints().find((ep) => ep.id === baseService);
  const preset = resolveServicePreset(baseService);
  // V1 patch-model-chat-test：用户指定了模型就测它，不要偷换成列表第一个
  const requestedModel = args.preferredModel?.trim();
  const discoveredFirstModel = requestedModel && isTextChatModelId(requestedModel)
    ? requestedModel
...
export function createStudioServer(initialConfig: ProjectConfig, root: string, overrides: { readonly nodeImageGenerator?: NodeImageDeps } = {}) {
  ...
  // InkOS V2: adaptation-first workflow engine + artifact store (see docs/v2).
  app.route("/api/v2", createV2Router(root));
  let cachedConfig = initialConfig;
  const activeConfirmedTasks = new Map<string, AbortController>();
  // 每会话在飞 POST /api/v1/agent 计数（V1 patch-refresh-stop-button）
  const runningAgentChatRequests = new Map<string, number>();
  ...
  // --- Story ledger workspace（V1 patch-story-ledger-ui 移植） ---
  const STORY_LEDGER_EDITABLE_FILES = new Set([
    "current_state.md",
    "particle_ledger.md",
    "subplot_board.md",
  ]);
  async function readStoryLedgerFile(storyDir: string, file: string, fallback = ""): Promise<string> {
    try {
      return await readFile(join(storyDir, file), "utf-8");
    } catch {
      return fallback;
    }
  }

  app.get("/api/v1/books/:id/story-ledger", async (c) => {
    const id = c.req.param("id");
    if (!isSafeBookId(id)) return c.json({ error: "Invalid book id" }, 400);
    const storyDir = join(state.bookDir(id), "story");
    const [pendingHooks, hooksRaw, archiveRaw, currentState, particleLedger, subplotBoard, auditReport] = await Promise.all([
      readStoryLedgerFile(storyDir, "pending_hooks.md"),
      readStoryLedgerFile(storyDir, "state/hooks.json", "{\"hooks\":[]}"),
      readStoryLedgerFile(storyDir, "state/hooks-archive.json", "{\"hooks\":[]}"),
      readStoryLedgerFile(storyDir, "current_state.md"),
      readStoryLedgerFile(storyDir, "particle_ledger.md"),
      readStoryLedgerFile(storyDir, "subplot_board.md"),
      readStoryLedgerFile(storyDir, "hooks_audit_report.md"),
    ]);
    let hooks: unknown[] = [];
    let archive: unknown = { hooks: [] };
    try {
      hooks = (JSON.parse(hooksRaw) as { hooks?: unknown[] }).hooks ?? [];
    } catch { /* corrupt hooks.json → empty */ }
    try {
      archive = JSON.parse(archiveRaw);
    } catch { /* corrupt archive → empty */ }
    return c.json({
      bookId: id,
      hooks,
      archive,
      pendingHooks,
      auditReport,
      files: { currentState, particleLedger, subplotBoard },
    });
  });

  app.put("/api/v1/books/:id/story-ledger/hooks/:hookId", async (c) => {
    const id = c.req.param("id");
    const hookId = c.req.param("hookId");
    if (!isSafeBookId(id) || !/^H-CH\d+-\d+(?:（[^）]+）)?$/.test(hookId)) {
      return c.json({ error: "Invalid book or hook id" }, 400);
    }
    const body = await c.req.json().catch(() => ({} as Record<string, unknown>));
    const status = typeof body.status === "string" ? body.status.trim() : "";
    const notes = typeof body.notes === "string" ? body.notes.trim().replaceAll("|", "｜") : "";
    if (!status || status.length > 64 || notes.length > 20000) {
      return c.json({ error: "Invalid status or notes" }, 400);
    }
    const storyDir = join(state.bookDir(id), "story");
    const pendingPath = join(storyDir, "pending_hooks.md");
    const hooksPath = join(storyDir, "state", "hooks.json");
    const [pending, hooksRaw] = await Promise.all([
      readFile(pendingPath, "utf-8"),
      readFile(hooksPath, "utf-8"),
    ]);
    const hooksDoc = JSON.parse(hooksRaw) as { hooks?: Array<{ hookId?: string; status?: string; notes?: string }> };
    const target = hooksDoc.hooks?.find((hook) => hook.hookId === hookId);
    if (!target) return c.json({ error: "Hook not found" }, 404);
    target.status = status;
    target.notes = notes;
    let changed = false;
    const pendingNext = pending.split(/\r?\n/).map((line) => {
      if (!line.startsWith("|")) return line;
      const cells = line.split("|");
      if ((cells[1] ?? "").trim() !== hookId || cells.length < 10) return line;
      cells[4] = ` ${status} `;
      cells[8] = ` ${notes} `;
      changed = true;
      return cells.join("|");
    }).join("\n");
    if (!changed) return c.json({ error: "Hook row missing from pending_hooks.md" }, 409);
    const stamp = new Date().toISOString().replace(/[:.]/g, "-");
    await Promise.all([
      writeFile(`${pendingPath}.pre-ledger-edit-${stamp}.bak`, pending, "utf-8"),
      writeFile(`${hooksPath}.pre-ledger-edit-${stamp}.bak`, hooksRaw, "utf-8"),
    ]);
    await Promise.all([
      writeFile(pendingPath, pendingNext, "utf-8"),
      writeFile(hooksPath, JSON.stringify(hooksDoc, null, 2), "utf-8"),
    ]);
    return c.json({ ok: true, hook: target });
  });

  app.put("/api/v1/books/:id/story-ledger/file/:file", async (c) => {
    const id = c.req.param("id");
    const file = c.req.param("file");
    if (!isSafeBookId(id) || !STORY_LEDGER_EDITABLE_FILES.has(file)) {
      return c.json({ error: "Invalid book id or ledger file" }, 400);
    }
    const body = await c.req.json().catch(() => ({} as Record<string, unknown>));
    const content = typeof body.content === "string" ? body.content : null;
    if (content === null || content.length > 2_000_000) {
      return c.json({ error: "Invalid or oversized content" }, 400);
    }
    const path = join(state.bookDir(id), "story", file);
    const previous = await readStoryLedgerFile(join(state.bookDir(id), "story"), file);
    const stamp = new Date().toISOString().replace(/[:.]/g, "-");
    await writeFile(`${path}.pre-ledger-edit-${stamp}.bak`, previous, "utf-8");
  ...
  // Probe whether an endpoint+model can read natively attached PDF files.
  // Runs one real (tiny) inference request and caches the verdict in
  // .inkos/file-capabilities.json keyed by provider|baseUrl|model.
  app.post("/api/v1/services/:service/file-capability-test", async (c) => {
    const service = c.req.param("service");
    const body = await c.req.json().catch(() => ({} as Record<string, unknown>));
    const language = await currentProjectLanguage();
    const reqModel = typeof body.model === "string" ? body.model.trim() : "";
    if (!reqModel) {
      return c.json({ ok: false, error: pick(language, "缺少 model 参数", "model is required") }, 400);
    }
    try {
      ...
      const result = await probeNativePdfCapability({
        projectRoot: root,
  ...
  app.post("/api/v1/services/:service/test", async (c) => {
    ...
    const probe = await probeServiceCapabilities({
      ...
      preferredModel: requestedModel || undefined,
      proxyUrl,
      language,
    });

    // V1 patch-model-chat-test：两步验证 —— 列表探测（probe）+ 真实推理（chat）。
    // 列表能拉 ≠ 模型能用；chat 一步必须真的出字才算通过。
    const connectionFailed = pick(language, "连接失败", "Connection failed");
    const probeStatus = {
      ok: probe.ok,
      models: probe.models?.length ?? 0,
      selectedModel: requestedModel || probe.selectedModel,
    ...
    const selectedModel = requestedModel || probe.selectedModel;
    if (!selectedModel || !isTextChatModelId(selectedModel)) {
      const error = pick(
        language,
        "模型列表探测通过，但没有指定可用于真实推理测试的文本模型。",
        "The model-list probe passed, but no text model was selected for the real inference test.",
      );
      return c.json({
        ok: false,
        error,
        ...
        selectedModel,
        probe: probeStatus,
        chat: { ok: false, model: selectedModel ?? "", error },
      });
    }

    const selectedApiFormat = apiFormat === "responses" || apiFormat === "chat" ? apiFormat : probe.apiFormat;
    const selectedStream = typeof stream === "boolean" ? stream : probe.stream;
    const chatStatus = await runServiceChatProbe({
      service,
      apiKey: apiKey?.trim() ?? "",
      baseUrl: resolvedBaseUrl,
      model: selectedModel,
    ...
    if (!chatStatus.ok) {
      const error = [
        pick(
          language,
          `模型列表探测通过（${probeStatus.models} 个模型），但 ${selectedModel} 的真实推理失败。`,
          `The model-list probe passed (${probeStatus.models} models), but real inference failed for ${selectedModel}.`,
        ),
        chatStatus.error,
      ].filter(Boolean).join("\n");
      return c.json({
  ...
  app.get("/api/v1/sessions/:sessionId", async (c) => {
    ...
    const task = await loadReconciledTaskSnapshot(sessionId);
    // chatRunning（V1 patch-refresh-stop-button）：普通聊天轮在飞标记，
    // 前端刷新后据此恢复停止按钮与 SSE 流。
    const chatRunning = runningAgentChatRequests.has(sessionId);
    return c.json({ session, chatRunning, ...(task ? { task } : {}) });
  });
  ...
  // 每会话在飞聊天轮计数（V1 patch-refresh-stop-button）：普通聊天轮没有
  // 任务快照，前端刷新后靠 GET /sessions/:id 的 chatRunning 恢复停止按钮。
  app.use("/api/v1/agent", async (c, next) => {
    ...
    let chatSessionId = "";
    try {
      const body = await c.req.json<{ sessionId?: unknown }>();
      if (typeof body?.sessionId === "string") chatSessionId = body.sessionId.trim();
    } catch {
      // Malformed body: let the route handler report the error.
    }
    if (!chatSessionId) {
      await next();
      return;
    }
    runningAgentChatRequests.set(chatSessionId, (runningAgentChatRequests.get(chatSessionId) ?? 0) + 1);
    try {
      await next();
    } finally {
      const pending = runningAgentChatRequests.get(chatSessionId) ?? 0;
      if (pending <= 1) runningAgentChatRequests.delete(chatSessionId);
      else runningAgentChatRequests.set(chatSessionId, pending - 1);
    }
  });
  ...
  app.post("/api/v1/agent", async (c) => {
    const {
      ...
      model: rawReqModel,
      service: reqService,
      thinkingLevel: reqThinkingLevel,
      interactionMode: reqInteractionMode,
    } = await c.req.json<{
      instruction: string;
      activeBookId?: string;
      sessionId?: string;
    ...
    // 思考强度三通道：请求体显式字段 > "/think 高 …" 指令前缀 > "model@high" 后缀。
    const parsedDirective = parseThinkingDirective(rawInstruction, rawReqModel);
    const instruction = parsedDirective.instruction;
    const reqModel = parsedDirective.model;
    const explicitThinking = ["minimal", "low", "medium", "high", "xhigh"].includes(String(reqThinkingLevel))
      ? (reqThinkingLevel as ThinkingLevel)
      : undefined;
    const thinkingLevel = explicitThinking ?? parsedDirective.level;
    const thinkingFromModelSuffix = parsedDirective.fromModelSuffix;
    // Ask 模式：只读会话——生产工具从工具表剔除（read/research/propose_action 保留）。
    const interactionMode: "agent" | "ask" = reqInteractionMode === "ask" ? "ask" : "agent";
    if (!instruction?.trim()) {
    ...
    try {
      ...
      if (!resolvedModel) {
        ...
        // V1 patch-refresh-state：回退用配置里选定的服务（rawConfig.service），
        // 不是服务数组第一个 —— 否则多服务配置下默认模型解析到错误端点。
        const configuredService = typeof rawConfig.service === "string" ? rawConfig.service.trim() : "";
        const selectedServiceEntry = servicesArr.find((entry) => serviceConfigKey(entry) === configuredService)
          ?? (configuredService && !isCustomServiceId(configuredService)
            ? { service: configuredService }
            : undefined)
          ?? servicesArr[0];
        if (selectedServiceEntry?.service && defaultModel && isTextChatModelId(defaultModel)) {
          try {
            const resolved = await resolveServiceModel(
              serviceConfigKey(selectedServiceEntry),
              defaultModel,
              root,
              selectedServiceEntry.baseUrl,
              selectedServiceEntry.apiFormat,
            );
            resolvedModel = resolved.model;
            resolvedApiKey = resolved.apiKey;
          } catch { /* fall through */ }
      ...
      // 仅本地 Cursor 反代才附带私有 cursor_mode；其它端点绝不能收到该字段。
      const resolvedServiceId = configuredEntry?.service ?? reqService ?? config.llm.service ?? "";
      const resolvedServiceBaseUrl = configuredEntry?.baseUrl
        ?? (typeof (config.llm as { baseUrl?: unknown }).baseUrl === "string" ? String((config.llm as { baseUrl?: unknown }).baseUrl) : "");
      const cursorModeForProxy = isCursorProxyService(resolvedServiceId, resolvedServiceBaseUrl)
        ? interactionMode
        : undefined;
      ...
      // extra：模型后缀思考强度是"端点支持 reasoning_effort"的显式声明，
      // writer/auditor/reviser 等 sub-agent 调用也携带；cursor_mode 仅反代。
      const pipelineExtra: Record<string, unknown> = {
        ...((config.llm as { extra?: Record<string, unknown> }).extra ?? {}),
        ...(thinkingLevel && thinkingFromModelSuffix
          ? { reasoning_effort: thinkingLevel === "xhigh" ? "high" : thinkingLevel }
          : {}),
        ...(cursorModeForProxy ? { cursor_mode: cursorModeForProxy } : {}),
      };
      const pipelineClient = (reqService && reqModel && resolvedModel)
        ? createLLMClient({
            ...
            ...(Object.keys(pipelineExtra).length > 0 ? { extra: pipelineExtra } : {}),
          } as any)
        : client;
      ...
      // V1 patch-direct-chapter-actions：口语化审计/修订命令确定性直连，
      // 不再依赖第三方模型是否愿意发 sub_agent 工具调用。
      const directChapterAction = agentBookId && sessionKind === "book"
        ? parseStudioDirectChapterAction(instruction)
        : null;
      const confirmedIntent: RequestedIntent | undefined = directChapterAction?.intent
        ?? (isWriteNextProductionRequest({ instruction, agentBookId, sessionKind, actionSource, requestedIntent })
          ? "write_next"
      ...
      const result = await runAgentSession(
        {
          ...
          ...(cursorModeForProxy ? { cursorMode: cursorModeForProxy } : {}),
          // Ask 模式 = 只读会话：生产工具剔除（与后台任务并行保护共用同一开关）。
          ...(backgroundTask || interactionMode === "ask"
            ? {
                ...(backgroundTask
...
export async function startStudioServer(
  ...
  if (options?.staticDir) {
    ...
    // 每次请求从磁盘读：dist 重建后 hashed bundle 会换名，启动时缓存字符串会导致
    // 服务器发旧 HTML 去引用已删除的 bundle（404 白屏）。
    const indexPath = joinPath(options.staticDir!, "index.html");
    if (existsSync(indexPath)) {
      app.get("*", async (c) => {
        if (c.req.path.startsWith("/api/v1/")) return c.notFound();
        // 顶层独立页面（如 /story-ledger.html）：磁盘上存在的真实文件优先于 SPA 兜底
        if (/^\/[\w-]+\.[a-z0-9]+$/i.test(c.req.path)) {
          try {
            const content = await readFileFs(joinPath(options.staticDir!, c.req.path.slice(1)));
        ...
        // no-cache：HTML 引用带内容哈希的 bundle，浏览器缓存旧 HTML 会指向已删除的资源
