...
export function extractRelevantThreads(pendingHooksRaw: string, subplotBoardRaw: string): string {
  const hookRows = parseMarkdownTableRows(pendingHooksRaw)
    ...
    .map((row) => {
      ...
      // 长线欠账（V1 patch-hooks-pipeline）：回收前先自然强化一次，避免读者忘线
      const longSilent = row.some((cell) => /长线欠账|已沉默\s*\d+\s*章/.test(cell));
      const reinforcement = longSilent
        ? " [回收前强化：若本章相关，先用旧物件、旧对白、意象或现实后果自然再提一次；不要直接解释谜底]"
        : "";
      return `- ${row[0]}: ${detail}${reinforcement}`;
    });
...
export function formatRecyclableHooks(
  ...
  // 长篇项目经常同时挂着六条以上的陈旧 hook：窗口放宽到 12，且沉默 ≥5 章的
  // hook 明确要求"回收前先不动声色地再提一次"（V1 patch-hooks-pipeline）。
  const topSlice = hooks.slice(0, 12);
  const lines = topSlice.map((hook) => {
    ...
    const reinforcement = silence >= 5
      ? (language === "en"
        ? " [reinforce subtly before payoff: echo an object, line, image, or consequence without explaining it]"
        : " [回收前强化：先用物件、旧对白、意象或后果不动声色地再提一次，不要直接解释]")
      : "";
    return language === "en"
      ? `- ${hook.hookId} "${payoff}" — status=${hook.status}, silent ${silence} ch${core}${reinforcement}`
      : `- ${hook.hookId} "${payoff}" — 状态=${hook.status}，已沉默 ${silence} 章${core}${reinforcement}`;
  });
