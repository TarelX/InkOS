...
function parseHash(hash: string): HashRoute {
  ...
  const adaptationMatch = path.match(/^v2\/([^/]+)$/);
  if (adaptationMatch) return { page: "adaptation", bookId: decodeURIComponent(adaptationMatch[1]) };
...
function routeToHash(route: HashRoute): string {
  switch (route.page) {
    ...
    case "adaptation": return `#/v2/${encodeURIComponent(route.bookId)}`;
    default: return "";
  }
}
...
const HASH_PAGES = new Set(["dashboard", "chat", "book", "book-settings", "book-create", "services", "project-settings", "service-detail", "translation", "import", "play", "film", "flow", "film-author", "film-studio", "adaptation"]);
