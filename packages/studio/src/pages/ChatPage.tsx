...
// 与服务端 MAX_AGENT_ATTACHMENT_BYTES 保持一致（Office/PDF 文档友好）
...
export function ChatPage({ activeBookId, mode = activeBookId ? "book" : "book-create", nav, theme, t, sse: _sse }: ChatPageProps) {
  ...
  const interactionMode = useChatStore((s) => s.interactionMode);
  // -- Store actions --
  ...
  const setInteractionMode = useChatStore((s) => s.setInteractionMode);
  ...
  // V1 patch-refresh-state：模型列表加载中不做回退选择，避免把用户/配置的
  // 自定义模型错误覆盖成"第一个可枚举模型"。
  useEffect(() => {
    if (!serviceConfigLoaded || servicesLoading || bankModelsLoading || customModelsLoading) return;
  ...
  }, [configuredModelSelection, groupedModels, selectedModel, selectedService, serviceConfigLoaded, servicesLoading, bankModelsLoading, customModelsLoading, setSelectedModel]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (activeBookId) {
        ...
        if (ids.length > 0) {
          // V1 patch-refresh-state：优先恢复该书最后激活的会话
          let last: string | null = null;
          try { last = localStorage.getItem(`inkos:last-session:${activeBookId}`); } catch { /* ignore */ }
          const target = last && ids.includes(last) ? last : ids[0];
          activateSession(target);
          await loadSessionDetail(target);
          return;
        }
          ...
          <div className="flex items-start gap-2">
            ...
            >
              ...
              <div className={`flex items-center gap-2 px-3 pb-2 border-t pt-1.5 ${isWorkbench ? "border-[#eef2f8]" : "border-border/20"}`}>
                ...
                {/* V2：思考强度（自动/最低/低/中/高/极高） */}
                <DropdownMenu>
                  <DropdownMenuTrigger className="flex items-center gap-1 px-2 py-1.5 rounded-md hover:bg-muted text-[14px] text-muted-foreground transition-colors cursor-pointer">
                    <span>{isZh ? "思考" : "Think"}</span>
                    <span className="font-medium text-foreground">
                      {thinkingLevel
                        ? (isZh
                            ? ({ minimal: "最低", low: "低", medium: "中", high: "高", xhigh: "极高" }[thinkingLevel] ?? thinkingLevel)
                            : thinkingLevel)
                        : (isZh ? "自动" : "Auto")}
                    </span>
                  ...
                  <DropdownMenuContent align="start" className="min-w-[120px]">
                    {[
                      { id: null, zh: "自动", en: "Auto" },
                      { id: "minimal", zh: "最低", en: "Minimal" },
                      { id: "low", zh: "低", en: "Low" },
                      ...
                      <DropdownMenuItem
                        ...
                        className={thinkingLevel === item.id ? "text-primary font-medium" : ""}
                      >
                ...
                {/* V2：Agent / Ask 模式（Ask = 只读，不执行生产动作） */}
                <DropdownMenu>
                  <DropdownMenuTrigger
                    className="flex items-center gap-1 px-2 py-1.5 rounded-md hover:bg-muted text-[14px] text-muted-foreground transition-colors cursor-pointer"
                    title={isZh ? "Ask 模式为只读：不执行建书/写章等生产动作" : "Ask mode is read-only: no production actions"}
                  >
                    <span>{isZh ? "模式" : "Mode"}</span>
                    <span className={`font-medium ${interactionMode === "ask" ? "text-amber-500" : "text-foreground"}`}>
                      {interactionMode === "ask" ? "Ask" : "Agent"}
                    </span>
                    <ChevronDown size={14} className="text-muted-foreground" />
                  </DropdownMenuTrigger>
                  <DropdownMenuContent align="start" className="min-w-[130px]">
                    <DropdownMenuItem onClick={() => setInteractionMode("agent")} className={interactionMode === "agent" ? "text-primary font-medium" : ""}>
                      Agent{isZh ? "（可执行）" : ""}
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => setInteractionMode("ask")} className={interactionMode === "ask" ? "text-primary font-medium" : ""}>
                      Ask{isZh ? "（只读）" : ""}
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
