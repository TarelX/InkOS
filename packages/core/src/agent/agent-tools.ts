    ...
    async execute(
      ...
      try {
        ...
        if (agent === "architect" && err instanceof ArchitectIncompleteFoundationError) {
          // V1 patch-architect-retry：architect_incomplete 是真实失败，文案必须
          // 诚实 —— 书没建成，partial content 只存在于失败快照里。isError 由
          // agent-session 的 afterToolCall 钩子按 details.kind 标记。
          const missing = err.missing.join(", ");
          const sessionIsZh = inferLanguage(err.message) !== "en";
          return textResult(
            sessionIsZh
              ? [
                  "基础设定生成不完整，建书失败。",
                  `缺失 section: ${missing}`,
                  "书籍尚未加入创作列表。已生成的 partial content 仅保存在此失败任务快照中；补齐全部基础设定后才会完成建书。",
                ].join("\n")
              : [
                  "The foundation is incomplete, so book creation failed.",
                  `Missing sections: ${missing}`,
                  "The book has not been added to the creations list. Partial content exists only in this failed task snapshot; book creation completes only after every foundation section is filled.",
                ].join("\n"),
            {
